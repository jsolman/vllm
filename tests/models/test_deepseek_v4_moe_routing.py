...
def _make_config(*, scoring_func: str, n_group: int | None, topk_group: int | None):
    ...
    moe.renormalize = config.norm_topk_prob
    moe.n_activated_experts = config.num_experts_per_tok
...
def test_deepseek_v4_sigmoid_preserves_grouped_routing(
    ...
    monkeypatch.setattr(deepseek_v4_model, "FusedMoEFactory", _FakeFusedMoE)
