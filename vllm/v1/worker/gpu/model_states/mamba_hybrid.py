...
class MambaHybridModelState(DefaultModelState):
    ...
    def add_request(self, req_index: int, new_req_data: NewRequestData) -> None:
        ...
        if self._align_mode:
            ...
            # the smallest prefix-cacheable group block size, which would
            # index the mamba block table in the wrong units and read past
            # the row on a prefix-cache hit (silently restoring the wrong
            # recurrent state on short hits).
